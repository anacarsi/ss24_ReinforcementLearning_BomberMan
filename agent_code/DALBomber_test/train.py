...
LOG_LEVEL = INFO
TRANSITION_HISTORY_SIZE = int(os.getenv("TRANSITION_HISTORY_SIZE"))
REQUIRE_HISTORY = int(os.getenv("REQUIRE_HISTORY"))
ACTIONS = ["UP", "RIGHT", "DOWN", "LEFT", "WAIT", "BOMB"]
# Transition = namedtuple("Transition", ("state", "action", "next_state", "reward"))


BATCH_SIZE = int(os.getenv("BATCH_SIZE"))
# TODO: BOMB_SHAPE = int(os.getenv("BOMB_SHAPE"))

EPSILON_INIT = float(os.getenv("EPSILON_INIT"))
LR_INIT = float(os.getenv("LR_INIT"))
EPSILON_DECAY_FACTOR = float(os.getenv("EPSILON_DECAY_FACTOR"))
LR_DECAY_FACTOR = float(os.getenv("LR_DECAY_FACTOR"))
...
def setup_training(self: DummySelf):
    ...
    self.started_training_obj = self.last_saved
    self.amount_saved = 0
    ...
    self.optimizer = torch.optim.Adam(self.model.parameters(), LR_INIT)  # 00001 # 1e-5 for later
...
def train(self: DummySelf):
    ...
    if isinstance(minibatch, int):
        ...
        return 
    if self.trained % 200 == 1:
        np.save(f"{self.started_training}_loss_history.npy", np.array(self.loss))
        np.save(f"{self.started_training}_score_history.npy", np.array(self.score))
        self.target_network.load_state_dict(self.model.state_dict())
        self.logger.debug("200 train")
        self.logger.info(f"trained at: {self.trained}, {self.transitions.wrapped} {self.transitions.index}, round: {self.current_last_round} epsilon down to: {self.epsilon} lr at {self.optimizer.param_groups[0]["lr"]} at {str(datetime.now()-self.started_training_obj)}")
...
def end_of_round(self: DummySelf, last_game_state: dict, last_action: str, events: List[str]):
    ...
    self.current_last_round = last_game_state["round"]
    ...
    if self.trained% 1000 == 999:
        ...
        self.epsilon = max(0.001,self.epsilon * EPSILON_DECAY_FACTOR)
        for param in self.optimizer.param_groups:
            param["lr"] = param["lr"]* LR_DECAY_FACTOR
