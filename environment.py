    ...
    def update_bombs(self):
        ...
        for bomb in self.bombs:
            if bomb.timer <= 0:
                ...
                # Create explosion
                screen_coords = [
                    (s.GRID_OFFSET[0] + s.GRID_SIZE * x, s.GRID_OFFSET[1] + s.GRID_SIZE * y) for (x, y) in blast_coords
                ]
...
class GUI:
    ...
    def render(self):
        ...
        for x in range(self.world.arena.shape[1]):
            for y in range(self.world.arena.shape[0]):
                if self.world.arena[x, y] == -1:
                    self.screen.blit(
                        self.t_wall, (s.GRID_OFFSET[0] + s.GRID_SIZE * x, s.GRID_OFFSET[1] + s.GRID_SIZE * y)
                    )
                if self.world.arena[x, y] == 1:
                    self.screen.blit(
                        self.t_crate, (s.GRID_OFFSET[0] + s.GRID_SIZE * x, s.GRID_OFFSET[1] + s.GRID_SIZE * y)
                    )
        self.render_text(
        ...
        for coin in self.world.coins:
            if coin.collectable:
                coin.render(
                    self.screen, s.GRID_OFFSET[0] + s.GRID_SIZE * coin.x, s.GRID_OFFSET[1] + s.GRID_SIZE * coin.y
                )

        # Agents
        for agent in self.world.active_agents:
            agent.render(
                self.screen, s.GRID_OFFSET[0] + s.GRID_SIZE * agent.x, s.GRID_OFFSET[1] + s.GRID_SIZE * agent.y
            )
        ...
        for i, a in enumerate(agents):
            ...
            self.render_text(
                f"{a.score:d}", 830, y_base + 50 * i, (255, 255, 255), valign="center", halign="right", size="big"
            )
            self.render_text(
                f"{a.total_score:d}", 890, y_base + 50 * i, (64, 64, 64), valign="center", halign="right", size="big"
            )
        ...
        if not self.world.running:
            ...
            self.render_text(leading.display_name, x_center, 320, color, valign="top", halign="center", size="huge")
            self.render_text("has won the round!", x_center, 350, color, valign="top", halign="center", size="big")
